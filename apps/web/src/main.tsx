...
function App() {
  ...
  async function handleAudioFileUpload(event: React.ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    if (!file) return;

    // Validate file type
    const allowed = ["audio/wav", "audio/wave", "audio/x-wav", "audio/mpeg", "audio/mp3", "audio/ogg", "audio/webm", "audio/mp4", "audio/m4a", "audio/x-m4a", "audio/aac"];
    if (!allowed.some((t) => file.type.startsWith(t.split("/")[0])) && !file.name.match(/\.(wav|mp3|ogg|webm|m4a|aac|flac|wma)$/i)) {
      alert("Unsupported audio format. Please upload WAV, MP3, OGG, WebM, or M4A files.");
      return;
    }
    ...
    try {
      ...
      const res = await apiClient.uploadAudioAndInteract(file, sid, language, file.name);
      ...
      alert("Could not process the audio file. Please try again or use voice/text input.");
    } finally {
      setProcessing(false);
