...
class TurnRecord(BaseModel):
    ...
    spoken_response_indic: str = Field(..., description="Spoken assistant response in Hindi/dialect")
...
class CallSessionRecord(BaseModel):
    ...
    session_id: str = Field(..., min_length=1, max_length=64, description="Unique session ID")
    call_uuid: Optional[str] = Field(None, description="FreeSWITCH call UUID")
    phone_hash: Optional[str] = Field(None, description="Salted SHA-256 phone hash")
    
    # Language & dialect
    language: str = Field(default="hi")
    dialect: Optional[str] = Field(default="bhojpuri_mixed")

    # Final FSM state
    final_state: str = Field(..., description="Terminal state e.g. COMPLETED, HANGUP, ERROR")
