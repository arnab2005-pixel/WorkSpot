...
class DpiuApplicationRecord(BaseModel):
    ...
    application_id: str = Field(..., description="Unique application identifier e.g. 'DPIU-UP-VAR-2026-0001'")
    ...
    selected_trade: str = Field(..., description="Selected vocational trade / enterprise")
    recommended_course_qp: Optional[str] = Field(None, description="Recommended NSQF QP code")
    ...
    requested_asset: Optional[str] = Field(None, description="Machinery / tool-kit requested")
    estimated_capital_inr: int = Field(default=0, ge=0)
    subsidy_eligible_amount_inr: int = Field(default=0, ge=0)
    
    # DPDP Audit proof
    voice_consent_audio_vault_ref: str = Field(..., description="Audio vault reference of explicit consent")
