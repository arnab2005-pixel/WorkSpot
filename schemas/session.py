...
class ProfilingSlot(BaseModel):
    ...
    slot_name: Literal[
        "CONSENT", "LOCATION", "TRADE", "MOBILITY", "INTENT", "EDUCATION"
    ]
...
class SessionData(BaseModel):
    ...
    call_uuid: str | None = Field(default=None, description="FreeSWITCH call UUID")
    phone_hash: str | None = Field(
        None, description="Hashed phone for beneficiary lookup"
    )
    ...
    def get_slot(self, slot_name: str) -> ProfilingSlot | None:
        """Get a slot by name."""
    ...
    def is_slot_filled(self, slot_name: str) -> bool:
        ...
        return slot is not None and slot.status in (
            SlotStatus.FILLED,
...
class SessionUpdate(BaseModel):
    ...
    current_state: FSMState | None = None
    slots: dict[str, ProfilingSlot] | None = None
    extracted_entities: dict[str, Any] | None = None
    recommendations: list[dict[str, Any]] | None = None
    audio_vault_refs: list[str] | None = None
...
class ExtractedSlots(BaseModel):
    ...
    detected_trade: str | None = Field(
        None, description="Identified artisan/vocational skill in Hindi/English"
    )
    prior_experience_years: float | None = Field(default=0.0, ge=0.0)
    mobility_radius_km: int | None = Field(default=15, ge=0, le=100)
    employment_intent: str | None = Field(
        None, pattern=r"^(WAGE|SELF_EMPLOYMENT|HYBRID)$"
    )
