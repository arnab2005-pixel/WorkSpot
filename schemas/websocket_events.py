...
class StartCallEvent(WSIncomingMessage):
    ...
    call_uuid: Optional[str] = Field(None, description="FreeSWITCH call UUID")
...
class BargeInEvent(WSIncomingMessage):
    ...
    timestamp_ms: int = Field(..., description="Offset in current TTS playback where barge-in occurred")
...
class TranscriptFinalEvent(WSOutgoingMessage):
    ...
    is_hallucinated: bool = Field(default=False, description="Flagged by hallucination protection")
...
class TTSChunkEvent(WSOutgoingMessage):
    ...
    text: Optional[str] = Field(None, description="Text this chunk corresponds to")
...
def create_outgoing_event(event_type: WSMessageType, session_id: str, **kwargs) -> OutgoingEvent:
...
TELEPHONY_FRAME_BYTES = TELEPHONY_FRAME_SAMPLES * TELEPHONY_CHANNELS * (TELEPHONY_BIT_DEPTH // 8)  # 320
